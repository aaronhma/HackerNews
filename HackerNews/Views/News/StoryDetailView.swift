...
struct StoryDetailView: View {
	...
	@State private var showingReportPostFinishedAlert = false
	@State private var selectedOffensiveOption = 0
	@State private var additionalContextText = ""
		...
		GeometryReader { geometry in
			VStack {
				...
				NavigationStack {
							...
							Menu {
								...
								Section {
									...
									Button(role: .destructive) {
										showingReportPostSheet.toggle()
									} label: {
										Label("Report Content", systemImage: "flag.badge.ellipsis")
									}
								}
							} label: {
					...
					.alert("Thanks for your report. We're on it.", isPresented: $showingReportPostFinishedAlert) {
					}
					.sheet(isPresented: $showingReportPostSheet) {
						NavigationStack {
							List {
								Section {
									Text("What's your concern with this content?")
										.font(.largeTitle)
										.bold()
								}
								
								Section("Pick an option") {
									Button {
										if selectedOffensiveOption == 1 {
											selectedOffensiveOption = 0
										} else {
											selectedOffensiveOption = 1
										}
									} label: {
										HStack {
									...
									Button {
										if selectedOffensiveOption == 2 {
											selectedOffensiveOption = 0
										} else {
											selectedOffensiveOption = 2
										}
									} label: {
										HStack {
											Text("Hate speech")
									...
									Button {
										if selectedOffensiveOption == 3 {
											selectedOffensiveOption = 0
										} else {
											selectedOffensiveOption = 3
										}
									} label: {
										HStack {
									...
									Button {
										if selectedOffensiveOption == 4 {
											selectedOffensiveOption = 0
										} else {
											selectedOffensiveOption = 4
										}
									} label: {
										HStack {
											Text("Mature content")
									...
									Button {
										if selectedOffensiveOption == 5 {
											selectedOffensiveOption = 0
										} else {
											selectedOffensiveOption = 5
										}
									} label: {
										HStack {
											Text("Other (provide context)")
								...
								Section("Additional Context") {
									TextField("Anything else?", text: $additionalContextText)
								}
										...
										NavigationStack {
											List {
												...
												Section {
													...
													Text("Your Hacker News account is used to prevent spam.")
												}
											}
											.navigationTitle("Additional Information Sent")
											.navigationBarTitleDisplayMode(.inline)
										}
									} label: {
										Label("Additional Information Sent", systemImage: "paperplane")
									}
								} header: {
									Text("Additional Information Sent")
								} footer: {
									Text("This article and your Hacker News account is used to prevent spam and review reports.")
								}
								
								Section {
									Button {
										showingReportPostSheet.toggle()
										showingReportPostFinishedAlert = true
									} label: {
										Text("Send Report")
											.font(.title2)
											.frame(maxWidth: .infinity)
									...
									.frame(height: 40)
									.disabled(selectedOffensiveOption == 0)
								}
							}
